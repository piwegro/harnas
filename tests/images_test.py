...
import unittest
from exc import ImageNotFoundError
...
class images_test(unittest.TestCase):
    ...
    def test_get_image_by_id(self):
        db.connect()
        db.execute("DELETE FROM piwegro.images", ())
        db.execute("INSERT INTO piwegro.images (id, offer_id, original, thumbnail, preview) VALUES (%s, %s, %s, %s, %s)", (1, 1, 'test', 'test', 'test'))
    ...
    def test_associate_with_offer(self):
        db.connect()
        db.execute("DELETE FROM piwegro.images", ())
        db.execute("DELETE FROM piwegro.offers WHERE id = 2", ())
        db.execute("INSERT INTO offers (id, seller_id, name, description, price, currency, images, created_at) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)", (2, 'KyumBFaY66ZdS3oG7fPZQZycKyC2', 'Półeczka', 'Bardzo ładna półeczka we wspaniałym stanie', 4, 'HAR', '{}', '2022-10-31 18:32:19.000000'))
        db.execute("INSERT INTO piwegro.images (id, offer_id, original, thumbnail, preview) VALUES (%s, %s, %s, %s, %s)", (1, 1, 'test', 'test', 'test'))
        img = Image.get_image_by_id(1)
        img.associate_with_offer(2)
        img = Image.get_image_by_id(1)
        result = db.fetch("SELECT offer_id FROM piwegro.images WHERE id = 1", ())
